...
// Eliminar los registros relacionados en la tabla carrito_saco
$sql_eliminar_carrito_saco = "DELETE FROM carrito_saco WHERE id_saco = ?";
$stmt_eliminar_carrito_saco = $conn->prepare($sql_eliminar_carrito_saco);
if (!$stmt_eliminar_carrito_saco) {
    $respuesta = array('mensaje' => "Error en la preparación de la consulta: " . $conn->error);
} else {
    $stmt_eliminar_carrito_saco->bind_param("i", $id_producto);
    if ($stmt_eliminar_carrito_saco->execute()) {
        ...
        $respuesta = array('mensaje' => "Error al eliminar los registros relacionados en carrito_saco: " . $stmt_eliminar_carrito_saco->error);
    }
    $stmt_eliminar_carrito_saco->close();
}
